/**
 * @fileoverview Download and upload files and directories in the
...
async function upload(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!fileAuthorizationHelper.isSanitized(dirPath)) {
    res
      .status(400)
      .json({ message: `Path ${dirPath} is not a valid directory` });
    return;
  }
  ...
  if (!permissions.write) {
    res
      .status(400)
      .json({
        message: `User ${user.username} does not have write permissions for ${dirPath}`,
      });
    return;
  }
