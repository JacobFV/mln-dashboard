# MWM with relaxing the uniqueness constraint and includes ties (if any); number of matches can be more than MWM
import networkx as nx
import os
import re
def maximalWeightedMatchingRelaxedWithTiesExtender(previousResult, bipartiteGraph, resultFile, newLayerStatus):
    matchedCommunities = []
...
####
    ...
    for left, right in match:
        ...
        left_match = [right] # which nodes are higher from left
        ...
        for i,j in g[right].items():
            if i != left:
                if right_max < j['weight']:
                    right_match = [i]
                    right_max = j['weight']
                elif right_max == j['weight']:
                    right_match.append(i)
        ...
        for k in matchingTable:
            for i in range(0, len(matchingTable[k])):
                if k[0] == 1:
                    ...
                    r.write("<{0},{1};{2}>\n".format(matchingTable[k][i][1], k[1], content[1]))
    extend(previousResult, resultFile, bipartiteGraph)

def maximalWeightedMatchingRelaxedWithTiesGenerator(bipartiteGraph, resultFile):
    print("in generator")
    g = nx.Graph()
    edges = []
    bipartite_data = open(bipartiteGraph, "r").read().splitlines()
    ...
    matchingTable = {}
    for left, right in match: # check if higher or equal weighted edge is available
        ...
        right_match = [] # which nodes are higher from right
        for i,j in g[left].items():
            if i != right: # (left, right) needs to considered ONLY once
                if left_max < j['weight']:
                    left_match = [i]
                    left_max = j['weight'] # we JUST CHECK for higher edge weight; NOT the sum of edges
                elif left_max == j['weight']:
                    left_match.append(i)
        for i,j in g[right].items():
            if i != left:
        ...
        if left_max > right_max: # is the unique weight from left higher or from right ?
            ...
            for i in right_match:
                if right in matchingTable:
                    ...
                    matchingTable[right] = [i]
        elif left_max == right_max: # if the unique weight from both side is same then include both sides matches
            for i in left_match:
                if left in matchingTable:
                    matchingTable[left].append(i)
                else:
