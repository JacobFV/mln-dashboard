...
####
    ...
    for left, right in match:
        left_max = round(g[left][right]['weight'], 3)
        right_max = round(g[left][right]['weight'], 3)
        for i,j in g[left].items():
            if left_max == round(j['weight'], 3):
...
#                    r.write("<{0},{1};{2}>\n".format(k[1], matchingTable[k][i][1], bipartiteGraph))
                    r.write("<{0},{1};{2}>\n".format(k[1], matchingTable[k][i][1], round(g[k][matchingTable[k][i]]['weight'], 3)))
            #    else:
