...
import os
import re
def maximalWeightedMatchingExtender(previousResult, bipartiteGraph, resultFile, newLayerStatus):
    matchedCommunities = []
...
#    extend(previousResult, resultFile, str(g[ii[0]][ii[1]]['weight']))

def maximalWeightedMatchingGenerator(bipartiteGraph, resultFile):
    g = nx.Graph()
    edges = []
    bipartite_data = open(bipartiteGraph, "r").read().splitlines()
    for line in bipartite_data:
